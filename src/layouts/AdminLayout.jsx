...
const navItems = [
  ...
  { to: '/admin/polls', label: 'Polls', icon: '🗳️' },
...
export function AdminLayout() {
  ...
  async function handleSignOut() {
    ...
    catch { toast.error('Sign out error') }
  }
  ...
  return (
    <div className="min-h-screen bg-surface flex">
      <aside className="w-64 border-r border-border flex flex-col fixed h-screen z-20 bg-surface-1">
        <div className="p-6 border-b border-border">
          <div className="flex items-center gap-3">
            {SITE_CONFIG.logoUrl ? (
              <img src={SITE_CONFIG.logoUrl} alt={SITE_CONFIG.siteName} className="w-8 h-8 rounded-lg object-cover flex-shrink-0" />
            ) : (
      ...
      <div className="flex-1 ml-64 flex flex-col">
        <motion.main initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="flex-1">
          <Outlet />
        </motion.main>
        <SiteFooter />
      </div>
      ...
      <Toaster position="top-right" toastOptions={{
        style: { background: '#181c27', color: '#e8eaf2', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px' },
      }} />
    </div>
  )
}
