...
export function VoteCard({ card, selected, disabled, onClick }) {
  return (
    <motion.button
      ...
      className={`relative group flex flex-col rounded-2xl overflow-hidden cursor-pointer transition-all duration-200 text-left w-full ${
    ...
    >
      <div className={`relative w-full h-full rounded-2xl transition-all duration-300 ${
        selected ? 'shadow-card-selected' : 'shadow-card hover:shadow-card-hover'
      }`}
        style={{
          outline: selected ? '2px solid #22c55e' : disabled ? '1px solid rgba(255,255,255,0.04)' : '1px solid rgba(255,255,255,0.08)',
      ...
      >
        ...
        <div className={`px-4 py-3 transition-colors duration-300 rounded-b-2xl ${
          selected ? 'bg-success/10 border-t border-success/20' : 'bg-surface-2 border-t border-border'
        }`}>
          <p className={`font-semibold text-sm leading-tight transition-colors duration-200 ${selected ? 'text-success' : 'text-text-primary'}`}>
            {card.title}
          </p>
        </div>
      </div>
