...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="glass-panel px-3 py-2 text-sm">
      ...
      <p className="text-accent">{payload[0].value} votes</p>
    </div>
  )
}

function GeoBarChart({ data, emptyText, dataKey = 'label' }) {
  const chartData = Object.entries(data).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([label, count]) => ({ [dataKey]: label, count }))
  if (chartData.length === 0) return <p className="text-text-muted text-sm text-center py-8">{emptyText}</p>
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={chartData} layout="vertical" margin={{ left: 10, right: 20 }}>
        ...
        <YAxis type="category" dataKey={dataKey} stroke="#8b90a4" fontSize={12} width={110} tickLine={false} axisLine={false} />
...
export function CountryChart({ data }) {
  return <GeoBarChart data={data} emptyText="No country data" dataKey="country" />
}

export function CityChart({ data }) {
  return <GeoBarChart data={data} emptyText="No city data" dataKey="city" />
}

export function TimelineChart({ data }) {
  const chartData = Object.entries(data).sort((a, b) => a[0].localeCompare(b[0])).map(([date, count]) => ({
    date: new Date(date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' }),
    count,
  }))
  if (chartData.length === 0) return <p className="text-text-muted text-sm text-center py-8">No activity data</p>
  return (
    <ResponsiveContainer width="100%" height={280}>
