...
const CardRow = forwardRef(function CardRow({ card, onUpdate, onRemove }, ref) {
  return (
    <motion.div ref={ref} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      exit={{ opacity: 0, scale: 0.95 }} className="glass-panel p-3 flex flex-col gap-2 relative">
      ...
      <ImageUrlField imageUrl={card.imageUrl} position={card.imagePosition}
        onChange={(url) => onUpdate({ ...card, imageUrl: url })}
        onPositionChange={(position) => onUpdate({ ...card, imagePosition: position })} />
      <input type="text" value={card.title} onChange={(e) => onUpdate({ ...card, title: e.target.value })}
        placeholder="Card name" className="input-field text-sm py-2" />
    </motion.div>
  )
})
...
export function CardEditor({ cards, onChange }) {
  ...
  function updateCard(index, updated) {
    const next = [...cards]; next[index] = updated; onChange(next)
  }
  function removeCard(index) { onChange(cards.filter((_, i) => i !== index)) }

  return (
    <div className="space-y-3">
      <p className="text-xs text-text-muted">
        Paste a direct image link for each card (e.g. from imgbb.com, imgur.com, or any image host)
      </p>
