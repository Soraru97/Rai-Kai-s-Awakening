...
export function ImageUrlField({ imageUrl, position, onChange, onPositionChange }) {
  ...
  function handleBlur() { onChange(localUrl.trim()) }
  ...
  function handlePointerDown(e) {
    ...
    function handlePointerMove(ev) {
      if (!hasDragged && (Math.abs(ev.clientX - startX) > DRAG_THRESHOLD || Math.abs(ev.clientY - startY) > DRAG_THRESHOLD)) {
        hasDragged = true
      }
      if (hasDragged) updatePositionFromEvent(ev.clientX, ev.clientY)
    }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div ref={frameRef} onPointerDown={handlePointerDown}
        ...
        {localUrl && !error ? (
          <img src={localUrl} alt="" draggable={false}
          ...
          <div className="text-center p-4">
            ...
            <p className="text-xs text-text-muted">{error ? 'Failed to load image' : 'Paste an image link below'}</p>
          </div>
        )}
        ...
        {localUrl && !error && (
          <button type="button"
            ...
            className={`absolute top-1.5 left-1.5 z-10 p-1.5 rounded-lg backdrop-blur-sm transition-colors ${
              isPositioning ? 'bg-accent text-white' : 'bg-surface-2/80 text-text-muted hover:text-text-primary'
            }`}
            title={isPositioning ? 'Done' : 'Reposition image'}>
            {isPositioning ? (
        ...
        <AnimatePresence>
          {isPositioning && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <p className="text-[10px] text-text-secondary">Drag to reposition</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>

      <input type="url" value={localUrl} onChange={(e) => setLocalUrl(e.target.value)} onBlur={handleBlur}
        placeholder="https://example.com/image.jpg" className="input-field text-xs py-2" />
    </div>
  )
}
