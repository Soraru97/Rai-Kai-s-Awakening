...
export const Input = forwardRef(function Input({ label, error, hint, className = '', ...props }, ref) {
  return (
    <div className="w-full">
      ...
      <input ref={ref} className={`input-field ${error ? 'border-red-500 focus:border-red-500' : ''} ${className}`} {...props} />
...
export const Textarea = forwardRef(function Textarea({ label, error, hint, rows = 4, className = '', ...props }, ref) {
  return (
    <div className="w-full">
      {label && <label className="label">{label}</label>}
...
export function DateInput({ label, error, ...props }) {
  return (
    <div className="w-full">
      {label && <label className="label">{label}</label>}
      <input type="datetime-local" className={`input-field ${error ? 'border-red-500' : ''} [color-scheme:dark]`} {...props} />
      {error && <p className="mt-1.5 text-xs text-red-400">{error}</p>}
    </div>
  )
}
...
export function NumberInput({ label, error, hint, min = 1, max = 50, ...props }) {
  return (
    <div className="w-full">
      {label && <label className="label">{label}</label>}
      <input type="number" min={min} max={max} className={`input-field ${error ? 'border-red-500' : ''}`} {...props} />
      {error && <p className="mt-1.5 text-xs text-red-400">{error}</p>}
      {hint && !error && <p className="mt-1.5 text-xs text-text-muted">{hint}</p>}
    </div>
  )
  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer select-none">
      <div className="relative">
        ...
        <div className={`w-11 h-6 rounded-full transition-colors duration-200 ${checked ? 'bg-accent' : 'bg-surface-4'}`} />
