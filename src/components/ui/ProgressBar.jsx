    ...
    <div className={`w-full ${className}`}>
      {showLabel && (
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs text-text-secondary font-medium">Stage {value} of {max}</span>
          <span className="text-xs text-accent font-semibold">{Math.round(pct)}%</span>
        </div>
      )}
...
export function ResultBar({ votes, total, color = '#6c63ff' }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-sm font-mono text-text-secondary w-10 text-right">{Math.round(pct)}%</span>
    </div>
  )
}
